...
- Uses the 'gh' CLI to create the PR`,
  ...
  needsApproval: true,
    branchName: z.string().describe("The name of the new branch to create and push"),
    commitMessage: z.string().describe("The commit message"),
    prTitle: z.string().describe("The title of the pull request"),
    prBody: z.string().optional().describe("The body/description of the pull request"),
  }),
  execute: async (
    ...
    try {
      ...
      if (!branchRes.success && !branchRes.stderr.includes("already exists") && !branchRes.stderr.includes("already a branch")) {
        return { success: false, error: `Failed to create branch: ${branchRes.stderr}` };
      }

      // 2. Stage all changes
      await sandbox.exec("git add -A", cwd, TIMEOUT_MS, { signal: abortSignal });
      ...
      if (!commitRes.success && !commitRes.stdout.includes("nothing to commit") && !commitRes.stderr.includes("nothing to commit")) {
        return { success: false, error: `Failed to commit: ${commitRes.stdout} ${commitRes.stderr}` };
      }
      ...
      if (!pushRes.success) {
        return { success: false, error: `Failed to push: ${pushRes.stdout} ${pushRes.stderr}` };
      }
      ...
      return { success: false, error: `Error during commit and PR process: ${message}` };
    }
  },
});
