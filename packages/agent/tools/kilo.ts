...
class Context7Client {
    ...
    constructor(apiKey?: string) {
        ...
        if (!key) {
            throw new Error('Context7 API key is required. Provide it as parameter or set CONTEXT7_API_KEY environment variable.');
        }
        this.headers = {
    ...
    async call(method: string, params: Record<string, any> = {}): Promise<any> {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Context7 API call failed with status ${response.status}: ${errorText}`);
        }
